...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 16 23:03:45 2023
...
phiphi_ref = cosmo.raw_cl()["pp"][2:] #same as that from raw_cl
...
for cB_ind in range(len(cBarr)):
    ...
    cosmo.compute()
    cl = cosmo.raw_cl()["tt"][2:]
    dl = cl*ell*(ell+1)/(2*np.pi)
    
    axs[0].plot(ell, dl, alpha=1, label=f"$c_B$ = {cBarr[cB_ind]}")
    axs[1].plot(ell, 100*(dl-dl_ref)/dl_ref, alpha=1)
    ...
    pk = np.array(pk)
    axs2[0].loglog(karr, pk, alpha=1, label=f"$c_B$ = {cBarr[cB_ind]}")  
    axs2[1].plot(karr, 100*(pk-pk_ref)/pk, alpha=1)
...
tauarr= cosmo.get_background()["conf. time [Mpc]"]
cBtauarr = cosmo.get_background()["braiding_smg"]
omega_smg = cosmo.get_background()["(.)rho_smg"]/cosmo.get_background()["(.)rho_crit"]

fig3, axs3 = plt.subplots()
axs3.set_xlabel("conf. time [Mpc]")
axs3.loglog(tauarr, cBtauarr, label="$\\alpha_B$")
axs3.loglog(tauarr, omega_smg, label="$\Omega_{smg}$")
axs3.legend()